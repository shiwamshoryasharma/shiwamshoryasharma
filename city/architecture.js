...
export function designBuilding(repo) {
  ...
  const height=3.8+Math.min(9,Math.log2(1+(sourceFiles??0))*1.1)+Math.min(4,Math.log10(1+repo.bytes)*.45);
  const language=repo.language;
  const family=language==='Jupyter Notebook'?'Observatory campus':language==='Python'?'Research terraces':language==='TypeScript'?'Crystal spire':language==='JavaScript'?'Connected towers':['HTML','CSS','SCSS'].includes(language)?'Creative studios':'Engineering hub';
  const parts=[];
  const add=(x,z,w,d,y,h)=>parts.push({x,z,w,d,y,h});
  const flip=seed%2?1:-1;
  if(family==='Crystal spire'){
    add(0,0,3.6,3.4,0,height*.4);add(flip*.3,0,2.8,2.8,height*.4,height*.34);add(flip*.45,0,1.9,2.1,height*.74,height*.26);
  }else if(family==='Connected towers'){
    add(-1.1,0,1.6,3,0,height);add(1.1,.35,1.6,2.5,0,height*.76);add(0,.35,.6,1.1,height*.47,.65);
  }else if(family==='Research terraces'){
    if(seed%3===0){
  ...
  return {seed,height,sourceFiles,family,parts,roof:family==='Observatory campus'?'dome':family==='Crystal spire'?'spire':family==='Creative studios'?'garden':'solar'};
}
