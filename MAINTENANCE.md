...
Edit prose and project links in `README.md`, skill cards in `scripts/build_visuals.py`, and chart layouts in `scripts/update_profile.py`. The public profile prose intentionally omits employer, education, and contact information. The owner explicitly approved publishing the full supplied portrait, including its visible institution and certificate details. The requested spoiler uses a closed `<details>` element: its contents remain public and can be expanded or read in the source.
...
The Pages homepage is a full public portfolio with selected project summaries, engineering skills, a perspective portrait, interests, and verified GitHub, LinkedIn, and ORCID links. Its content is ordinary HTML and remains available without WebGL. Its prose omits employers, education, contact details and location; resume files and private repository metadata are excluded. The full supplied portrait is an explicitly approved exception for the institution and certificate visible within the image. The existing Vivy/Alice fan-art banner is reused in the interests section with attribution; no additional raster artwork was generated.

`city/portfolio.css` styles the responsive indigo/violet/ember theme. `portfolio.js` tilts the portrait on request; it does not run a continuous animation. The world loads its Three.js modules when approaching the viewport, and rendering stops when off screen or the document is hidden. Reduced-motion preference disables automatic orbit and resident movement; visitors may enable them explicitly.
...
`realm.js` adds instanced cobblestones, raised footpaths, crossings and flowers. Public buildings preserve their repository language/source-file architecture and corrected roofs. Standard rough materials and shared procedural stone, slate, wood, plaster and scale maps give surfaces continuous shading. Cottage geometry adds timber framing, recessed panes, shutters, stone steps, door hardware and chimneys. Entering travel or the hidden kingdom temporarily expands the world; returning restores the prior scroll position. Reduced-motion preference stops automatic orbit, NPC movement and the kingdom animation.
...
The header uses the full public name, Shiwam Shorya Sharma. `assets/profile.png` is copied unchanged from the supplied image; the main portrait uses `object-fit: contain` to preserve the entire composition. Its compact header thumbnail uses a cover crop. Both are staged by an explicit build allowlist.

The dragon has a curved neck, tapered animated tail, sculpted head and jaw, horns, teeth, claws, dorsal spines, scales and ribbed, scalloped wing membranes. One shared procedural model serves both town flyovers and the portal destination.
