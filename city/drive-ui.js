...
export function createDrivingUI(city,onOpen){
  ...
  function keyDown(event){
    if(!city.driving||event.target.closest('input,textarea,select,[contenteditable="true"]'))return;
  ...
  $('drive-daylight').addEventListener('click',()=>{$('theme').setAttribute('aria-pressed',String(city.toggleDaylight()));});
  const map=$('minimap'),ctx=map.getContext('2d');
  function minimap(car,world){
    const scale=210/(world.bounds*2),px=x=>120+x*scale,pz=z=>120+z*scale;
  ...
  city.onDriveUpdate(({car,parking,world,input})=>{
    if(!city.driving)return;
    $('drive-speed').textContent=String(Math.round(Math.abs(car.speed)*3.6)).padStart(2,'0');$('drive-gear').textContent=Math.abs(car.speed)<.12?(parking.canOpen?'P':'N'):car.speed<0?'R':'D';
    $('steering-wheel').style.transform=`rotate(${(input.steer||0)*35}deg)`;
    const target=goal||parking.repo;
    if(parking.restricted){$('drive-destination').textContent=parking.label;$('drive-hint').textContent='Restricted grounds. No parking or entry.';}
    else if(target){const distance=Math.hypot(car.x-target.x,car.z-(target.z+world.parkingOffset));$('drive-destination').textContent=target.name;$('drive-hint').textContent=parking.canOpen?`Parked at ${parking.repo.name}. Press F to open.`:`${distance.toFixed(1)} m to the parking bay · hold Space to brake`;}
    const label=parking.restricted?'ACCESS DENIED':parking.canOpen?'PARKED · REPOSITORY READY':parking.inBay?'BRAKE TO PARK':'FIND A MINT P BAY';
